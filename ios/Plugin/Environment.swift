...
@objc public class Environment: NSObject {
    ...
    @objc public func get() throws -> [String: Any] {
        ...
        if jsonObject == nil {
            ...
            if bundlePath == nil {
                throw NSError(
                    domain: "Unable to load environment.json. Check if the file is available in the iOS environment folder or run npx cap copy (if you configured the environment copy)",
                    code: 0
                )
            }
